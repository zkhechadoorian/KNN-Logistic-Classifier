...
def main() -> None:
    """
    Quick EDA driver for the three supported datasets.
    Run e.g.:
        python Functions.py --dataset adult
        python Functions.py --dataset rice
        python Functions.py --dataset mushroom
    """
    # ────────────────────────────────────────────────────────────────
    # 0.  Standard libs
    # ────────────────────────────────────────────────────────────────
    ...
    # 1.  CLI arguments
    # ────────────────────────────────────────────────────────────────
    parser = argparse.ArgumentParser(description="Basic EDA on a UCI dataset")
    parser.add_argument(
        "--dataset",
        choices=["adult", "rice", "mushroom"],
        default="adult",
    ...
    file_map = {
        "adult":    os.path.join(data_dir, "adult.data"),
        "rice":     os.path.join(data_dir, "Rice_Cammeo_Osmancik.arff.txt"),
        "mushroom": os.path.join(data_dir, "agaricus-lepiota.data"),
    }
    full_path = file_map[args.dataset]
    base_name = os.path.basename(full_path)   # "adult.data", etc.

    # ────────────────────────────────────────────────────────────────
    # 3.  Tiny helper: temporary chdir
    # ────────────────────────────────────────────────────────────────
    ...
    # ────────────────────────────────────────────────────────────────
    # 4.  Load the data (inside data_dir so readFile’s comparisons work)
    # ────────────────────────────────────────────────────────────────
    with workdir(data_dir):
        data, labels = readFile(base_name)

    data   = np.asarray(data,   dtype=float)
    labels = np.asarray(labels, dtype=int).reshape(-1, 1)

    combined   = np.hstack((data, labels))
    categories = [f"feature_{i+1}" for i in range(data.shape[1])]

    analyser = dataAnalysis(combined, categories, classifications=[0, 1])

    # ────────────────────────────────────────────────────────────────
    # 5.  Run the reports
    # ────────────────────────────────────────────────────────────────
    analyser.printLabelStats()
    analyser.printCategoryStats()
